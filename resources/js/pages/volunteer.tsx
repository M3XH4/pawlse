...
import { Heart, Users, MapPin, Check, Plus, ShieldCheck, Zap, HandHeart, Trophy, Award, ChevronRight, ChevronLeft, Info, CheckCircle2, Calendar, Clock, Utensils, User, Mail, Phone, Home } from 'lucide-react';
...
export default function VolunteerPage() {
    const { props } = usePage();
    const selectedEventFromState = props.selectedEvent as Event | null;
    ...
    useEffect(() => {
        if (selectedEventFromState) {
            // Scroll to top when coming from events page
            window.scrollTo({ top: 0, behavior: 'smooth' });

            if (selectedEventFromState.type === 'feeding') {
                // eslint-disable-next-line react-hooks/set-state-in-effect
                setFormData(prev => ({ ...prev, role: 'Food Carrier' }));
            } else if (selectedEventFromState.type === 'event') {
                // Auto-select role based on event category
                if (selectedEventFromState.category === 'Medical') {
                    setFormData(prev => ({ ...prev, role: 'Recorder' }));
                } else if (selectedEventFromState.category === 'Feeding') {
                    setFormData(prev => ({ ...prev, role: 'Food Carrier' }));
                }
                // For Social and Adoption events, let user choose their role
            }
        }
    ...
    return (
        <div className="min-h-screen bg-paw-bg font-quicksand">
                                                ...
                                                >
                                                    <div className="grid md:grid-cols-2 gap-8">
                                                        <div>
                                                            ...
                                                            <input required type="text" className="w-full p-6 bg-paw-bg border-2 border-transparent rounded-[24px] outline-none focus:border-paw-orange transition-all font-bold" placeholder="First & Last Name" name="fullName" value={formData.fullName} onChange={handleInputChange} />
                                                        </div>
                                                        <div>
                                                            <label className="block text-sm font-black text-gray-400 mb-2 uppercase tracking-widest">Mobile Number *</label>
                                                            <input required type="tel" className="w-full p-6 bg-paw-bg border-2 border-transparent rounded-[24px] outline-none focus:border-paw-orange transition-all font-bold" placeholder="09XX XXX XXXX" name="mobile" value={formData.mobile} onChange={handleInputChange} />
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div className="grid md:grid-cols-2 gap-8">
                                                        <div>
                                                            ...
                                                            <input required type="email" className="w-full p-6 bg-paw-bg border-2 border-transparent rounded-[24px] outline-none focus:border-paw-orange transition-all font-bold" placeholder="your@email.com" name="email" value={formData.email} onChange={handleInputChange} />
                                                        </div>
                                                        <div>
                                                            <label className="block text-sm font-black text-gray-400 mb-2 uppercase tracking-widest">Address *</label>
                                                            <input required type="text" className="w-full p-6 bg-paw-bg border-2 border-transparent rounded-[24px] outline-none focus:border-paw-orange transition-all font-bold" placeholder="City, Barangay" name="address" value={formData.address} onChange={handleInputChange} />
                                                        </div>
                                                    </div>

                                                    <div>
                                                        <label className="block text-sm font-black text-gray-400 mb-4 uppercase tracking-widest">
                                                            Preferred Role * {formData.role && <span className="text-paw-orange">({formData.role} selected)</span>}
                                                        </label>
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                            {VOLUNTEER_ROLES.map((r) => (
                                                                <button
                                                                    key={r.id}
                                                                    type="button"
                                                                    className={`p-6 rounded-2xl border-2 font-bold text-left transition-all ${formData.role === r.title
                                            ...
                                            {currentStep === 3 && (
                                                ...
                                                >
                                                    <div>
                                                        ...
                                                        <textarea required className="w-full p-6 bg-paw-bg border-2 border-transparent rounded-[24px] outline-none focus:border-paw-orange transition-all font-bold min-h-[150px]" placeholder="Tell us about your passion for helping stray animals..." name="why" value={formData.why} onChange={handleInputChange} />
                                                    </div>

                                                    <div>
                                                        <label className="block text-sm font-black text-gray-400 mb-2 uppercase tracking-widest">Previous Experience (Optional)</label>
                                                        <textarea className="w-full p-6 bg-paw-bg border-2 border-transparent rounded-[24px] outline-none focus:border-paw-orange transition-all font-bold min-h-[100px]" placeholder="Any experience with animals or volunteer work?" name="experience" value={formData.experience} onChange={handleInputChange} />
                                                    </div>
                                                    ...
                                                    <div className="bg-green-50 border-2 border-green-200 rounded-3xl p-6">
                                                        ...
                                                        <div className="space-y-2 text-sm font-bold text-gray-700">
                                                            <p><span className="text-gray-500">Name:</span> {formData.fullName || 'Not provided'}</p>
                                                            <p><span className="text-gray-500">Role:</span> {formData.role || 'Not selected'}</p>
                                                            <p><span className="text-gray-500">Email:</span> {formData.email || 'Not provided'}</p>
                                                            <p><span className="text-gray-500">Mobile:</span> {formData.mobile || 'Not provided'}</p>
                                                        </div>
                                                    </div>
            ...
            <SubmissionReceipt
                ...
                items={[
                    { label: 'Full Name', value: formData.fullName, icon: <User size={20} /> },
                    { label: 'Mobile Number', value: formData.mobile, icon: <Phone size={20} /> },
                    { label: 'Email Address', value: formData.email, icon: <Mail size={20} /> },
                    { label: 'Address', value: formData.address, icon: <Home size={20} /> },
                    { label: 'Preferred Role', value: formData.role, icon: <HandHeart size={20} /> },
                    ...(selectedEventFromState ? [{
                    ...
                    { label: 'Motivation', value: formData.why, icon: <Heart size={20} /> }
                ]}
                type="success"
